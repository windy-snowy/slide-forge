...
**Q：`warn` 说"没有一行匹配核心文字"，但图里文字明明是对的？**
这是**整页 OCR 分段失败**，不是文字问题。真实案例（日系小清新那套 5 页）：

| | |
| --- | --- |
| 整页 OCR | 只认出标题 + 底部按钮 **2 行**，三张卡片里的文字一行都没检出 → 误报 |
| 整页放大 1.5× / 2× | 依然只有 2 行 → 放大**没用** |
| 把卡片区域**裁出来**单独 OCR | **3 行全读对** → 证明是分段失败 |

所以 `qa.py` 现在内置了**条带兜底**：当行命中率低于阈值时，自动把图按横向 4 条带切开分别 OCR 再合并，
然后重新评分。上面那页的匹配率因此从 **0.263 → 0.959**，5 页全部通过。

代价：每个"即将报警"的页会多花最多 4 次 OCR 调用（正常的页不受影响）。
不想要兜底就传 `probe_page(..., band_retry=False)`。

**Q：OCR 到底会不会造假通过？**
不会。三种情况在报告里区分得很清楚：
- `OCR 未返回可用文字` → 只做了几何检测（尺寸/空白/重复）
- `OCR 检出 N 行但没有一行匹配核心文字` → 可能是分段失败，也可能是文字真缺 → **请人工看图**
- `文本匹配率偏低 / 每行都检出了但字符匹配率偏低` → 文字级别的问题

`warn` 永远交给人判断，`fail` 只用于硬性问题（缺图、非 16:9、空白、重复）。
